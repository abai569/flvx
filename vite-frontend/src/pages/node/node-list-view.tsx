...
function SortableTableRow({
  ...
  return (
    <TableRow key={node.id} ref={setNodeRef} className="cursor-default" style={style}>
      ...
      <TableCell className={`whitespace-nowrap ${rowBg}`}><div className="flex justify-end"><span className="text-sm text-danger-600 dark:text-danger-400">{node.connectionStatus === "online" && realtimeNodeMetrics?.[node.id] ? formatTraffic((realtimeNodeMetrics?.[node.id]?.uploadTraffic || 0) + (realtimeNodeMetrics?.[node.id]?.downloadTraffic || 0)) : "-"}</span></div></TableCell>
      <TableCell className={`whitespace-nowrap ${rowBg}`}><div className="flex justify-end"><span className="text-sm text-success-700 dark:text-success-300">{node.connectionStatus === "online" && realtimeNodeMetrics?.[node.id] ? formatTraffic(realtimeNodeMetrics?.[node.id]?.uploadTraffic || 0) : "-"}</span></div></TableCell>
      <TableCell className={`whitespace-nowrap ${rowBg}`}><div className="flex justify-end"><span className="text-sm text-primary-700 dark:text-primary-300">{node.connectionStatus === "online" && realtimeNodeMetrics?.[node.id] ? formatTraffic(realtimeNodeMetrics?.[node.id]?.downloadTraffic || 0) : "-"}</span></div></TableCell>
